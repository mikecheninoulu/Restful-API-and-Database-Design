...
def resource_not_found(error):
    return create_error_response(400, "Malformed input format",
                                 "The format or the content of the input is incorrect")
...
class AllOrders(Resource):
    ...
    def get(self):
        ...
        envelope["collection"] = collection
        collection['version'] = "1.0"
        collection['href'] = api.url_for(AllOrders)
        collection['links'] = [
                                {'title': 'List of all orders in the sporthall',
...
class Order(Resource):
    ...
    def get(self, orderid):
        '''
        Get the body, the title and the id of a specific order.

        Returns status code 404 if the orderid does not exist in the database.

        INPUT PARAMETER
       : param str messageid: The id of the order to be retrieved from the
            system
        ...
        links['collection'] = {'href': api.url_for(Order, orderid=orderid),
                               'profile': FORUM_ORDER_PROFILE,
                               'type': COLLECTIONJSON}
        ...
        envelope['sportname'] = order_db['sportname']

        #RENDER
        return Response(json.dumps(envelope), 200,
                        mimetype=HAL+";"+FORUM_ORDER_PROFILE)
...
class Sports(Resource):
    ...
    def get(self):
       ...
       #Create the envelope
        ...
        envelope["collection"] = collection
        collection['links'] = [
                                {'title': 'List of all sports in the sporthall',
                                'rel': 'sports-all', 'href': api.url_for(Sports)
                                }
            ]
        collection['version'] = "1.0"
...
class Users(Resource):
    ...
    def get(self):
        ...
        Link relations used in items: orders

        Semantic descriptions used in items: nickname, registrationdate

        Link relations used in links: orders-all
